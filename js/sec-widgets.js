...
CQ.security.SecurityAdmin.saveUser = function(dialog, button, success) {
    ...
    try {
        ...
        var id = CQ.Ext.form.VTypes.makeAuthorizableId(emailField.getValue());
        if (!idField) {
...
CQ.security.PermissionsDetailPanel = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        var summaryPermGrid = new CQ.Ext.grid.EditorGridPanel({
            ...
            cm: new CQ.Ext.grid.ColumnModel({
                ...
                columns: [
                    {id: 'path', header: CQ.I18n.getMessage("Path"), width: 220, dataIndex: 'name', renderer: CQ.security.RightsPanel.TreeGrid.renderFullPath},
        ...
        var groupPermGrid = new CQ.Ext.grid.EditorGridPanel({
            ...
            cm: new CQ.Ext.grid.ColumnModel({
                columns: [
                    //{id: '_id', header: CQ.I18n.getMessage(""), dataIndex: '_id', width: 23, fixed: true, renderer: this.renderId},
                    ...
                    //{id: 'replicate', header: CQ.I18n.getMessage("Replicate"), dataIndex: 'replicate', renderer: this.renderGroupStatus},
                    memberCheck
                ],
                defaults: {
                    width: 65,
    ...
    renderOverviewStatus: function(scope, data, p, r, rowIndex, i, ds){
        var granted = p.get(data.id);
        var authorizableId = p.get("authorizableId");

        var declared = p.get("declared")[data.id];
        ...
        if (effective || nonEffective) {
            ...
            if (effective) {
                ...
                tips.push(CQ.security.RightsPanel.TreeGrid.renderTemplate(effective, function(item){
                    ...
                    out += "</td><td>";
                    out += "&nbsp;";
                    //out += (item == authorizableId ? "" : "(" + CQ.I18n.getMessage("Group") + ")");
                    //out += "(" + (item == authorizableId ? CQ.I18n.getMessage("Current") : CQ.I18n.getMessage("Group")) + ")";
                    out += "</td></tr>";
                    return out;
                }));
                tips.push("</table>");
        ...
        className.push(effective ? 'overriden' : 'inherited');
        className.push(nonEffective ? 'restriction' : 'norestriction' );
        return'<div ' + (qTips ? qTips : '') + '" class="x-grid3-check-col x-item-disabled ' + className.join('-') + '">&nbsp;<div/>';
        */
    ...
    /**
     * The renderer method for the grid checkbox cells.
     ...
     * @param ds
     */
    renderGroupStatus: function(scope, data, p, r, rowIndex, i, ds){
        var granted = p.get(data.id);
        var authorizableId = p.get("authorizableId");
        var restrictions =  p.get("restrictions");
...
CQ.security.RightsPanel.TreeGrid = CQ.Ext.extend(CQ.Ext.ux.maximgb.tg.EditorGridPanel, {
    ...
    constructor: function(config) {
        ...
        var cm = new CQ.Ext.grid.ColumnModel({
            ...
            columns: [
                {id: 'path', header: CQ.I18n.getMessage("Path"), width: 160, dataIndex: 'name', renderer: CQ.security.RightsPanel.TreeGrid.renderPath, editable: false},
                {id: 'read', header: CQ.I18n.getMessage("Read"), width: 75, dataIndex: 'read', renderer: CQ.security.RightsPanel.TreeGrid.renderCheckbox},
                {id: 'modify', header: CQ.I18n.getMessage("Modify"), width: 75, dataIndex: 'modify', renderer: CQ.security.RightsPanel.TreeGrid.renderCheckbox},
                {id: 'create', header: CQ.I18n.getMessage("Create"), width: 75, dataIndex: 'create', renderer: CQ.security.RightsPanel.TreeGrid.renderCheckbox},
                {id: 'delete', header: CQ.I18n.getMessage("Delete"), width: 75, dataIndex: 'delete', renderer: CQ.security.RightsPanel.TreeGrid.renderCheckbox},
                {id: 'acl_read', header: CQ.I18n.getMessage("Read ACL"), width: 75, dataIndex: 'acl_read', renderer: CQ.security.RightsPanel.TreeGrid.renderCheckbox},
                {id: 'acl_edit', header: CQ.I18n.getMessage("Edit ACL"), width: 75, dataIndex: 'acl_edit', renderer: CQ.security.RightsPanel.TreeGrid.renderCheckbox},
                {id: 'replicate', header: CQ.I18n.getMessage("Replicate"), width: 75, dataIndex: 'replicate', renderer: CQ.security.RightsPanel.TreeGrid.renderCheckbox},
                    ...
                    fn: function(grid, rowIndex, columnIndex, e) {
                        ...
                        if (columnId == 'details') { // path column
                            ...
                            if (record.data[fieldName] === "") {
                                // TODO: find better solution to disable actions if ac not readable/editable
                                // ac information not readable
                                // -> don't refresh view
                            } else {
     ...
     * @private
     */
    storeChanged: function(store, rec, action) {
     ...
     * Remove the highlight for the all rows.
     * @param sm
     * @param rowIndex
     * @param colIndex
     */
    ...
    /**
     * Reset the store and load the intial state for the authorizableId in the store (record.id)
     * @param record
     */
    loadRecord: function(record) {
        if (record && record.id) {
    ...
    /**
     * @private
     * @param paths An array of paths to loaded/updated sequentially
     */
 ...
 * @param ds
 */
CQ.security.RightsPanel.TreeGrid.renderCheckbox= function(scope, data, p, r, rowIndex, i, ds) {
    var granted = p.get(data.id);
...
CQ.security.RightsPanel.TreeGrid.renderPath= function(scope, data, p, r, rowIndex, i, ds){
    var isLeaf = p.get('_is_leaf');
    ...
    out += p.get(CQ.shared.XSS.getXSSPropertyName("name"));
    out += '</span>';
    return out;
};

CQ.security.RightsPanel.TreeGrid.renderFullPath= function(scope, data, p, r, rowIndex, i, ds){
    var isLeaf = p.get('_is_leaf');
    var out = '<span class="' + (isLeaf ? 'x-tree-node-leaf' : 'x-tree-node-collapsed') + '">';
    out += '<img unselectable="on" class="x-tree-node-icon ' + (isLeaf ? 'file' : 'folder') + '" style="height:16px;" src="/libs/cq/ui/resources/0.gif"/> ';
    out += p.get('_id');
...
CQ.security.RightsPanel.TreeGrid.renderDetails= function(scope, data, p, r, rowIndex, i, ds) {
    // test if the editing session has permission to read the ac info for this node.
    if ("" === p.get("acl_read")) {
...
/**
 * As of CQ5.4 the replication application privilege can be edited in the
 * regular permission overview. the tab "Replication Privilege" is consequently
 * not present any more in the security admin.
 *
 * @deprecated
 */
CQ.security.PrivilegeEditor = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        var cfg = CQ.Util.applyDefaults(config, {
            ...
            "anchor":"100% 100%",
            "autoShow":true,
            "privilege":"wcm/core/privileges/replicate",
            "title": CQ.I18n.getMessage("Replication Privilege"),
            "dataPath":CQ.HTTP.externalize('/bin/security/privileges'),
       ...
       this.aclStore=  new CQ.Ext.data.Store({
        ...
        this.tree = new CQ.Ext.tree.TreePanel({
            ...
            "baseParams": { "predicate":"hierarchy" },
                "requestMethod":"GET",
                "dataUrl": CQ.HTTP.externalize(cfg.treePath)+CQ.HTTP.EXTENSION_JSON,
...
/**
 * As of CQ5.4 the HierarchyModification application privilege has
 * been deprecated and the "Privilege" tab in the security administration
 * rendered by this JS got removed.
 *
 * @deprecated
 */
CQ.security.Privileges = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        tb.add({
            "id":"cq-useradmin-privileges-save",
            "disabled":true,
            "text":CQ.I18n.getMessage("Save"),
            "tooltip": {
                "title":CQ.I18n.getMessage("Save Changes"),
    ...
    enableSaveButton: function(enable) {
        ...
        if (this.dirty != enable) {
            this.dirty = enable;
            var sB = CQ.Ext.getCmp("cq-useradmin-privileges-save");
            if (sB){
                if (enable) {
                    sB.enable();
                } else {
    ...
    saveHandler: function() {
        ...
        var url = CQ.HTTP.externalize("/libs/cq/security/privileges/simple")
...
/**
 * @deprecate Since CQ 5.4
 */
CQ.security.PermissionEditor = CQ.Ext.extend(CQ.Ext.Panel, {

    /**
    * Id of the current selected Authorizable as set via load
     * @private
     * @type String
     */
    authorizableId:null,
    ...
    constructor: function(config) {
        ...
        var cfg = CQ.Util.applyDefaults(config, {
            ...
            "dataPath":CQ.HTTP.externalize('/bin/security/authorization/acltable'),
            "treePath":"/bin/tree/ext",
            "treeRoot": {
                "name":"",
                "text":"CQ",
        ...
        this.tree = new CQ.Ext.tree.TreePanel({
            ...
            "root":new CQ.Ext.tree.AsyncTreeNode(cfg.treeRoot),
            "loader":new CQ.Ext.tree.TreeLoader({
                "baseParams": { "predicate":"useradmin" },
                "requestMethod":"GET",
                "dataUrl": CQ.HTTP.externalize(cfg.treePath)+CQ.HTTP.EXTENSION_JSON,
                "baseAttrs": { "singleClickExpand":true },
                "listeners": {
        ...
        cfg.items = [this.tree, this.grid];
        CQ.security.PermissionEditor.superclass.constructor.call(this, cfg);
    },
     ...
     * Creates a new Record if non existed so far
     *
     * @private
     */
    editACE: function() {
    ...
    /**
     * Modification listener registered at the Store:
     * Activates the ToolBar buttons.
     * @param store
     * @param rec
     * @param action
     * @private
...
CQ.security.PermissionEditor.COLUMNS_RENDERER = function(v) {
    ...
    var txt;
    if (!v || v == "deny") {
        txt = CQ.I18n.getMessage("deny");
        color = "red";
    } else {
        txt = CQ.I18n.getMessage("allow");
        color = "green";
    }
...
CQ.security.AclEditor = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        config = CQ.Util.applyDefaults(config, {
            ...
            "dataPath":"/bin/security/authorization/acltable" /* TODO: to be replace by request to cq-actions-servlet. */
        });

        // effective permissions for the authorizables selected
        this.aclStore = new CQ.security.data.AclStore({
            "proxy": new CQ.Ext.data.HttpProxy({
                "url":config.dataPath + ".effective" + CQ.HTTP.EXTENSION_JSON, /* TODO: to be replace by request to cq-actions-servlet. */
                "method":"GET"
            })
        });
...
CQ.security.Preferences = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        this.fieldCfgs = [{
            ...
            "fieldDescription":"Show or hide the global editing toolbar",
            "fieldLabel":"Editing Toolbar",
            "width":"350px",
            ...
            "items":[ new CQ.Ext.DataView({
                ...
                "tpl": new CQ.Ext.XTemplate(
                        '<tpl for=".">',
                            ...
                            '{[values[CQ.shared.XSS.getXSSPropertyName(\"name\")] == "" ? values.id : values[CQ.shared.XSS.getXSSPropertyName(\"name\")]]}',
                            '</div>',
                        '</tpl>'
                        ),
                "id":"user-selection",
